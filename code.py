...
def read_letter_in_file_probability(
    file_name_input_num, if_ukr_set_boolean_true, file_ending_name_int
):  # .txt _base64.txt _bz2_base64.txt
    file_ending_name = [".txt", "_base64.txt", "_bz2_base64.txt"]
    if if_ukr_set_boolean_true:
        total_chars_in_text_amount_length_in_func = 0
        with open(
            "destextes/"
            + str(file_name_input_num)
            + file_ending_name[file_ending_name_int],
            "r",
            encoding="utf8",
        ) as file:
            total_text = ""
            # LetterD = {}
            letter_amount_in_text_dict = {
                "А": 0,
                "Б": 0,
                "В": 0,
                "Г": 0,
    ...
    return (
        letter_amount_in_text_dict,
        total_chars_in_text_amount_length_in_func,
        alphabet_power,
    )
...
def execute(
    ...
    if if_bzip_bool_true:
        ...
        probability_of_letter_to_appear_dict = probability_of_letter_to_appear(
            amount_letters_to_appear, total_number_of_char_in_text
        )
        ...
        amount_of_information_in_the_text = (
            entropy_rounded_h_f * total_number_of_char_in_text
        ) * 0.125  # /8
        print(
            "b64_bzip Information:",
            round(amount_of_information_in_the_text, 3),
            "bytes",
        )
        file_size = get_file_size(
            "destextes/" + str(file_name_input_num) + "_bz2_base64." + file_type_list[0]
        )
        ...
        if if_ukr_set_boolean_true:
            ...
            file_size = get_file_size(
                "destextes/" + str(file_name_input_num) + "." + file_type_list[0]
            )
            print("file size:", file_size, "bytes")
            zip_size = get_file_size(
                "destextes/" + str(file_name_input_num) + "." + file_type_list[1]
            )
            print("zipped size:", zip_size, "bytes")
            rar_size = get_file_size(
                "destextes/" + str(file_name_input_num) + "." + file_type_list[2]
            )
            print("rared size:", rar_size, "bytes")
            xz_size = get_file_size(
                "destextes/" + str(file_name_input_num) + "." + file_type_list[3]
            )
            print("xzed size:", xz_size, "bytes")
            gzip_size = get_file_size(
                "destextes/" + str(file_name_input_num) + "." + file_type_list[4]
            )
            print("gzipped size:", gzip_size, "bytes")
            bzip_size = get_file_size(
                "destextes/" + str(file_name_input_num) + "." + file_type_list[5]
            )
            print("bzipped size:", bzip_size, "bytes")
            base64_file_size = get_file_size(
                "destextes/" + str(file_name_input_num) + "_base64." + file_type_list[0]
            )
            ...
            print(
                "b64 Information:", round(amount_of_information_in_the_text, 3), "bytes"
            )
            file_size = get_file_size(
                "destextes/" + str(file_name_input_num) + "_base64." + file_type_list[0]
            )
            print("b64 file size:", file_size, "bytes")
            zip_size = get_file_size(
                "destextes/" + str(file_name_input_num) + "_base64." + file_type_list[1]
            )
            print("b64 zipped size:", zip_size, "bytes")
            rar_size = get_file_size(
                "destextes/" + str(file_name_input_num) + "_base64." + file_type_list[2]
            )
            print("b64 rared size:", rar_size, "bytes")
            xz_size = get_file_size(
                "destextes/" + str(file_name_input_num) + "_base64." + file_type_list[3]
            )
            print("b64 xzed size:", xz_size, "bytes")
            gzip_size = get_file_size(
                "destextes/" + str(file_name_input_num) + "_base64." + file_type_list[4]
            )
            print("b64 gzipped size:", gzip_size, "bytes")
            bzip_size = get_file_size(
                "destextes/" + str(file_name_input_num) + "_base64." + file_type_list[5]
            )
...
def preetty_output(
    amount_letters_to_appear,
    probability_of_letter_to_appear_base64_dict,
    amount_letters_to_appear_base64,
    probability_of_letter_to_appear_dict,
    entropy_rounded_h_f,
    amount_of_information_in_the_text,
    file_size,
    ...
    bzip_size,
    base64fsz,
    entropy_rounded_base64_h_f,
    amount_of_information_in_the_base64_text,
    base64_zip_size,
    base64_rar_size,
    base64_xz_size,
    base64_gzip_size,
    ...
    for i in probability_of_letter_to_appear_dict:
        table.add_row(
            [
                "'" + i + "'",
                amount_letters_to_appear[i],
                probability_of_letter_to_appear_dict[i],
            ]
        )
    ...
    for i in amount_letters_to_appear_base64:
        tableb64.add_row(
            [
                "'" + i + "'",
                amount_letters_to_appear_base64[i],
                probability_of_letter_to_appear_base64_dict[i],
            ]
        )
    ...
    table2.add_row(
        [
            "B64T. Information Amount",
            (round(amount_of_information_in_the_base64_text, 3)),
            " bytes",
        ]
    )
...
(
    ...
    total_number_of_char_in_text,
    alph_power,
) = read_letter_in_file_probability(file_name_input_num, True, 0)
(
    probability_of_letter_to_appear_dict,
    entropy_rounded_h_f,
    amount_of_information_in_the_text,
...
(
    amount_letters_to_appear_base64,
    total_number_of_char_in_text_base64,
    base64_alph_power,
) = read_letter_in_file_probability(file_name_input_num, False, 1)
...
(
    amount_letters_to_appear_base64_bzip,
    total_number_of_char_in_text_base64_bzip,
    base64_alph_power_bzip,
) = read_letter_in_file_probability(file_name_input_num, False, 2)
# print(amount_letters_to_appear_base64)
(
    probability_of_letter_to_appear_base64_dict_bzip,
    entropy_rounded_base64_h_f_bzip,
    amount_of_information_in_the_base64_text_bzip,
    base64_file_size_bzip,
) = execute(
    amount_letters_to_appear_base64_bzip,
    total_number_of_char_in_text_base64_bzip,
    base64_alph_power_bzip,
